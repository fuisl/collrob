...
private:
   ...
   Real m_fAvoidStart;    // when avoidance starts blending in
   Real m_fAvoidFull;     // when avoidance is fully on (weight=1)
   Real m_fAvoidGain;     // scales avoidance vector magnitude

   Real m_fLastMaxProx;   // for debugging / optional use
   Real m_fProxMag;


   CVector2 ObstacleAvoidanceVector();
   CVector2 VectorToTargetBlob();
   CVector2 GoalVector();

   Real     AvoidanceWeight(Real fMaxProx) const;
};
